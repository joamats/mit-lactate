...
print(f"{len(df1)} sepsis stays ({demo1})")
...
print(f"{len(df2)} stays with sepsis and lactate day 1 ({demo2})")
...
print(f"{len(df3)} stays with sepsis, lactate day 1 & 2, and LoS > 24h ({demo3})")

df4 = df3[df3.age >= 18]
print(f"Removed {len(df3) - len(df4)} stays with non-adult patient")
demo4 = print_demo(get_demography(df4))
print(f"{len(df4)} stays with sepsis, lactate day 1 & 2, LoS > 24h, adult patient ({demo4})")
...
print(f"{len(df5)} adults with sepsis, lactate day 1 & 2, LoS > 24h, adult patient, 1 stay per patient ({demo5})")
