...
def get_binned_data(df):
    ...
    # Combining the races into non-white and white categores
...
def get_table2(dfs):
    ...
    # mortality_in, los_icu_hours, mech_vent_overall_yes, rrt_overall_yes, vasopressor_overall_yes, transfusion_overall_yes
    # the column names will be the bins of lactate_day1
    cols = ["N", "mortality_in", "los_icu_hours", "mech_vent", "rrt",
            "vasopressor", 'transfusion_overall_yes']
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-i", "--input", default=Path('data/cohorts/cohort_eICU_lac1.csv').absolute(), help="input csv file")
    parser.add_argument(
        "-o", "--output", default=Path("results/table2/table2_eICU_lac1.csv").absolute(), help="output csv file")
