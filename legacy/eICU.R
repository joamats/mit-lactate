...
df <- read_csv('data/cohorts/cohort_eICU_lac1.csv', show_col_types = FALSE)
...
# Race/Ethnicity
df$race_group <- ifelse(df$race_group %in% c("White"), "White", "Non-White")
#df$race_group <- factor(df$race_group, levels = c("White", "Black", "Hispanic", "Asian"),
...
label(df$mech_vent) <- "Mechanical Ventilation (entire stay)"

# RRT
df$rrt <- factor(df$rrt, levels=c(1,0), labels=c("Received", "Did not receive"))
label(df$rrt) <- "RRT (entire stay)"
...
label(df$vasopressor) <- "Vasopressor(s) (entire stay)"
...
tbl1 <- table1(~ sex_female+ age + age_ranges + race_group + year +
                 ...
                 fluids_overall_yes + fluids_yes + fluids_sum_day1 + fluids_sum_day2 | race_group,
               data=df,
               render.missing=NULL,
...
t1flex(tbl1) %>% save_as_docx(path="results/table1/eICU_lac1_race_group.docx")
