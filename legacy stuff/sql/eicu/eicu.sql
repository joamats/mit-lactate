WITH
  ...
  SELECT
    ...
    MIN(ifnull(labresult,
        0)) AS min_lactate_24_hr,
    MAX(ifnull(labresult,
        0)) AS max_lactate_24_hr,
    AVG(ifnull(labresult,
        0)) AS avg_lactate_24_hr,
    AVG(ifnull(POWER(labresult,3),
        0)) AS m_3,
    AVG(ifnull(POWER(labresult,4),
        0)) AS m_4,
    STDDEV_POP(ifnull(labresult,
        0)) AS std_lactate_24_hr,
  FROM
...
, icu_type as (
  SELECT patientunitstayid, REGEXP_SUBSTR(diagnosisString, '[^|]+') as icu_type FROM `physionet-data.eicu_crd.diagnosis`
)
...
SELECT
    ...
    e.m_4,
    e.std_lactate_24_hr,
    l.charlson_comorbidity_index as cci,
    (SELECT COUNT(*) FROM turing-chess-353519.lactate_project.connect4 as B WHERE B.patientunitstayid = l.patientUnitStayID) as vasso,
    i.icu_type,
  ...
  JOIN
    cci AS l
  ON
    e.patientUnitStayID = l.patientUnitStayID
  JOIN
    icu_type as i
  ON 
   i.patientUnitStayID = l.patientUnitStayID
