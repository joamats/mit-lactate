...
-- There are some unresolved issues here: it is considered that spontaneous breathing begins to occur and that the status is active. The later appearance of a non-invasive or invasive recording can be considered as a cessation of autonomic breathing, as the modification takes longer and is therefore considered later.
-- The results are also presented here using the first version of the code for the time being.
-- can't process multiple subquery, so we split them and drop them finally
drop table if exists `bidmc-datathon.team8.ventaliation_info_time_0_eicu`;
create table `bidmc-datathon.team8.ventaliation_info_time_0_eicu` as
...
drop table if exists `bidmc-datathon.team8.ventaliation_info_use_4_eicu`;
create table `bidmc-datathon.team8.ventaliation_info_use_4_eicu` as
...
, ventaliation_info_time_0 as (
    select *
    from `bidmc-datathon.team8.ventaliation_info_time_0_eicu`
)
...
drop table if exists `bidmc-datathon.team8.pivoted_vent_eicu`;
create table `bidmc-datathon.team8.pivoted_vent_eicu` as
...
with ventaliation_info_use_4 as (
    select *
    from `bidmc-datathon.team8.ventaliation_info_use_4_eicu`
)
...
drop table if exists `bidmc-datathon.team8.pivoted_vent_part1_eicu`;
create table `bidmc-datathon.team8.pivoted_vent_part1_eicu` as
...
drop table if exists `bidmc-datathon.team8.pivoted_vent_part2_eicu`;
create table `bidmc-datathon.team8.pivoted_vent_part2_eicu` as
...
with ventilation_20 as (
	...
	from `bidmc-datathon.team8.pivoted_vent_part1_eicu`
)
...
drop table if exists `bidmc-datathon.team8.pivoted_vent_part34_eicu`;
create table `bidmc-datathon.team8.pivoted_vent_part34_eicu` as
...
with ventilation_30 as (
	...
	from `bidmc-datathon.team8.pivoted_vent_part2_eicu`
	where vent_flag = 0
)
...
drop table if exists `bidmc-datathon.team8.pivoted_vent_part56_eicu`;
create table `bidmc-datathon.team8.pivoted_vent_part56_eicu` as
...
with ventilation_50 as (
	...
	, ROW_NUMBER()
	OVER (partition by patientunitstayid order by cplitemoffset desc) as rn
	from `bidmc-datathon.team8.pivoted_vent_part34_eicu`
)
...
with ventilation_70 as (
	...
	from `bidmc-datathon.team8.pivoted_vent_part56_eicu`
)
